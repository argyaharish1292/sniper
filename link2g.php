          ...
          <!-- Topbar Form -->
            <form  action ="" method="post" class="form-inline ml-4">
              <div class="form-group mr-2">
                Query by : <select name="input_constraint" type="text" class="custom-select custom-select-sm">
                  <option value="siteid">SID</option>
                  <option value="vlan2g">VLAN</option>
                  <input type="text" name="input_query" placeholder="Input your Queries" class="form-control-sm bg-gray-300 border-0 text-gray-900" style="width:250px;" />
                </select>
              </div>
              <div class="form-group mr-5">
                <select name="input_bsc" class="custom-select custom-select-sm" placeholder="Input controller">
                  <?php
                    $select_query="MATCH (s:site) WHERE s.bsc <> 'NULL' RETURN DISTINCT [[s.bsc,s.bscid]] AS list;";
                    ...
                    foreach ($select_result->records() as $select_record){
                      $select_response =$select_record->get('list');
                        foreach ($select_response as $row){	
                          ?><option value="<?php echo $row[1]?>" ><?php echo $row[0]?></option><?php
                        }
                      }
                  ?>
                  </select>
              ...
              <div class="card shadow mb-4">
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                  <a href="#" data-toggle="collapse" data-target="#collapse22" aria-expanded="true" class="" data-toggle="tooltip" title="Click to Minimize/Expand">
			              ...
			              <span class="m-0 font-weight-bold text-danger">2G Route Viewer</span>
                  </a>
                  <script>
                    $(document).ready(function(){
                        $('[data-toggle="tooltip"]').tooltip();   
                ...
                <div class="collapse show" id="collapse22" style="">
                  <div class="card-body">
                    ...
                    <?php
                      //POST METHOD//
                      $input_query=@$_POST['input_query'];
                      $input_bsc=@$_POST['input_bsc'];
                      $input_constraint=@$_POST['input_constraint'];
                      $submit=@$_POST['sub'];
                      if ($submit){	//If submit button is pressed//
                        if ($input_query !=""){	//If there is query input on the form selection//
                          if ($input_constraint =='siteid'){
                            $query = "MATCH (a:site{siteid:'$input_query'}),(b:site{siteid:'$input_bsc'}), p=ShortestPath((a)-[*]-(b))
                                      WITH p,
                                      EXTRACT(n IN NODES(p)| [n.siteid,n.sitename,n.long,n.lat,n.ismetro,n.vlan2g,n.router2g,n.bsc,n.vlan3g,n.router3g,n.rnc,n.vlan4g,n.router4g]) as sites,
                                            EXTRACT(r IN RELATIONSHIPS(p)| [r.linkown,r.bw,r.linkconf,r.linkdetail,r.hopid]) as links
                                      RETURN links,sites;";
                                      //Pathfinder query (shortest path to controller by Site ID)//
                          }else{
                            $query = "MATCH (a:site),(b:site),p=ShortestPath((a)-[*]-(b)) 
                                      WHERE a.bscid='$input_bsc' AND a.vlan2g CONTAINS '$input_query' AND b.siteid='$input_bsc' AND a.siteid<>'$input_bsc'
                                      WITH p,
                                      EXTRACT(r IN RELATIONSHIPS(p)| [r.linkown,r.bw,r.linkconf,r.linkdetail,r.hopid]) as links,
                                      EXTRACT(n IN NODES(p)| [n.siteid,n.sitename,n.long,n.lat,n.ismetro,n.vlan2g,n.router2g,n.bsc,n.vlan3g,n.router3g,n.rnc,n.vlan4g,n.router4g]) as sites
                                      RETURN sites,links;";
							                        //Pathfinder query (shortest path to controller by VLAN 2G)//
                          }
                          ...
                          foreach ($result->records() as $record){ //For loops to assign each records as arrays//
                            ...
                            if ($record !==""){
                              ...
                              foreach ($response2 as $row){ //For each row, repeats ://
                                $site = createFeature(); //Create geoJson feature point//
                              ...
                              for($c=0;$c<$count; $c++){ //For loops as long as the node is not the last node, repeats ://
                                $d=$c+1;
                                $link = createLine(); //Create geoJson feature polyline//
                      ...
                      var map = L.map('mapid',{
                        ...
                        function onEachFeature(feature, layer) {
                            ...
                            +"<br> <b>VLAN 3G</b> :"+feature.properties.vlan3g+" - "+feature.properties.router3g+" - "+feature.properties.rnc+"<br> <b>VLAN 4G</b> :"+feature.properties.vlan4g+" - "+feature.properties.router4g);
                                layer.bindTooltip(feature.properties.siteid+" | "+feature.properties.vlan2g,{permanent: true, direction:'bottom',opacity:0.7});
                        }
                        //////////////////////////////////////////////////////////////////////////
                        ...
                        var arrow = L.geoJSON(geoJsonLine, {
                          ...
                          onEachFeature: function(feature,layer){
                            layer.bindPopup("<b>Hop ID</b>: "+feature.properties.hopid+"<br><b>Link Owner</b>: "+feature.properties.owner+"<br><b>Bandwidth</b>: "+feature.properties.bandwidth+"<br><b>Configurations</b>: "+feature.properties.config+"<br><b>Link Detail</b>:  "+feature.properties.detail);
                        ...
                        var zoom = L.TileJSON.createMap(map,{mapConfig: {
